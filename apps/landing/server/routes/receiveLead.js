/**
 * POST /api/receive-lead — save lead to MySQL (same contract as api/receive-lead-handler.php)
 */
...
const LOG_FILE = path.join(SYSTEM_ROOT, 'system-api.log');
...
export async function handleReceiveLead(req, res) {
  ...
  log(`receive-lead POST received (node) | name=${name.slice(0, 30)} | email=${email.slice(0, 40)}`);
  ...
  if (!isDatabaseConfigured()) {
    db_error_reason = 'Database not configured (.env DB_* or placeholders not replaced)';
  } else {
    try {
      ...
      if (!pool) {
        ...
        if (!tables || tables.length === 0) {
          db_error_reason = "Table 'leads' does not exist. Run database/schema-v3-completo.sql";
        } else {
          ...
          if (!is_dup) {
            ...
            const [result] = await pool.execute(
              `INSERT INTO leads (${cols}) VALUES (${place})`,
              values
            );
            lead_id = result.insertId;
            db_saved = true;
            inserted_new = true;
            log(`API: Lead received and saved to DB (id=${lead_id}) | Form: ${form_name}`);
          }
        }
      }
    } catch (e) {
      db_error_reason = e.message;
      log(`API receive-lead DB error: ${e.message}`);
    }
  }
